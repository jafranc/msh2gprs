...
{
	const char* what() const throw(){ return "Metis Error" ;}
};
...
public:
	...
	static vector<size_t> build_partitioning(const ConnectionMap<ConnType> & connections,
		...
		idx_t ret = METIS_PartGraphKway(&icount, &ncon, &xadj[0], &adj[0], &vwgt[0], &size[0],
				NULL/*&adjwgt[0]*/, &n_domains,
				NULL, NULL, options, &objval,
				&coarse_cell_id[0]);

		if(ret==METIS_ERROR || ret==METIS_ERROR_MEMORY ||  ret==METIS_ERROR)
			throw metis_error();
